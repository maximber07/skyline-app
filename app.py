...
# --- CALCULADORA DE CAMPO (INTEGRADA) ---
elif menu == "Calculadora de Campo":
    st.header("🧮 Calculadora Skyline para Empleados")
    st.markdown("Cotizador rápido de campo para empleados con las nuevas lógicas de descuento y restaurantes.")
    ...
    if btn_calc:
        ...
        st.markdown("""
        <style>
            .resumen-caja { background-color: #f8fafc; padding: 20px; border-radius: 10px; border-left: 5px solid #0f766e; box-shadow: 0 4px 6px rgba(0,0,0,0.1); }
        </style>
        """, unsafe_allow_html=True)
        ...
        st.write(f"**Cargos Adicionales (Hoods, Abanicos, Ductos):** ${total_adicionales_base:.2f}")
        ...
        st.write(f"**Tax ({q_estado.split(' ')[0]}):** ${tax_final:.2f}")
        st.markdown(f"<h2 style='color: #0f766e; margin-top:10px; border-top: 2px solid #cbd5e1; padding-top: 10px;'>TOTAL NETO A COBRAR: ${total_final_cobrar:.2f}</h2>", unsafe_allow_html=True)
